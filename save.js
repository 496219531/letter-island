...
(function(root){
  ...
  function validate(save){
    ...
    if(!Array.isArray(s.skills)||s.skills.length!==3||s.skills.some((k,i)=>!k||!(['sentences','speaking'].includes(s.learningMode)?root.ENGLISH_SENTENCES.some(tier=>tier.some(entry=>entry.word===k.code)):s.learningMode==='english'?root.ENGLISH_WORDS.some(tier=>tier.some(entry=>entry.word===k.code)):new RegExp('^'+['A','S','D'][i]+'[A-Z]{0,59}$').test(k.code))||!Number.isInteger(k.typed)||k.typed<0||k.typed>=k.code.length||!Number.isFinite(k.cd)||k.cd<0||k.duration!==[8,12,11][i]||!Number.isInteger(k.uses)))return false;
