...
(function(root){
  ...
  function validate(save){
    ...
    if(!Array.isArray(s.skills)||s.skills.length!==3||s.skills.some((k,i)=>!k||!(s.customBank?s.customBank.entries.some(e=>e.word===k.code):['sentences','speaking'].includes(s.learningMode)?Boolean(root.findSentenceEntry(k.code)):s.learningMode==='english'?Boolean(root.findWordEntry(k.code)):new RegExp('^'+['A','S','D'][i]+'[A-Z]{0,59}$').test(k.code))||!Number.isInteger(k.typed)||k.typed<0||k.typed>=k.code.length||(k.repeatsDone!==undefined&&(!Number.isInteger(k.repeatsDone)||k.repeatsDone<0||k.repeatsDone>4))||!Number.isFinite(k.cd)||k.cd<0||k.duration!==[8,12,11][i]||!Number.isInteger(k.uses)))return false;
  ...
  function restore(game,save){
    ...
    game.maxSpellLength=s.maxSpellLength??60;game.maxLearningLoad=s.maxLearningLoad??3;game.magicSlow=s.magicSlow??false;
    for(const skill of game.skills)skill.repeatsDone=skill.repeatsDone??0;
