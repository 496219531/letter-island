(function(root){
  ...
  function repository(storage){function list(){const raw=storage.getItem(KEY);if(!raw)return [];const items=JSON.parse(raw);if(!Array.isArray(items))throw Error('自定义词库数据异常，原记录未覆盖。');if(items.every(validate))return items;
    ...
    return {list,saveClassified({id,name,entries}){
    ...
    },save(group){const all=list(),i=all.findIndex(g=>g.id===group.id),existing=new Map((i>=0?all[i].entries:[]).map(e=>[e.word,e]));const rows=dedupe(group.entries.map(e=>entry(e,group.kind)),group.kind).map(e=>mergeFields(existing.get(e.word),e));const clean={id:group.id||Date.now().toString(36)+'-'+Math.random().toString(36).slice(2),name:String(group.name||'').trim(),kind:group.kind,entries:rows,...(group.familyId?{familyId:group.familyId}:{} )};if(!validate(clean))throw Error('请填写分组名称，并加入1至'+MAX_ENTRIES+'条有效内容。');if(i>=0)all[i]=clean;else{if(all.length>=MAX_GROUPS)throw Error('最多保存'+MAX_GROUPS+'个分类。');all.push(clean);}storage.setItem(KEY,JSON.stringify(all));return clean;},moveEntry({sourceId,sourceWord,input,kind}){
      ...
      if(destinationIndex>=0){const destination=all[destinationIndex],updatedDestination={...destination,entries:dedupe([...destination.entries,replacement],kind)},next=all.flatMap((group,index)=>index===destinationIndex?[updatedDestination]:index===sourceIndex?(remaining.length?[{...group,entries:remaining}]:[]):[group]);commit(next);return updatedDestination;}
      ...
      const sourceUpdated={...source,familyId:family,entries:remaining},destination={id:Date.now().toString(36)+'-'+Math.random().toString(36).slice(2),name:base+' · '+(kind==='word'?'单词词组':'句子'),kind,familyId:family,entries:[replacement]},next=[...all];next[sourceIndex]=sourceUpdated;next.push(destination);commit(next);return destination;
